...
const Navbar = () => {
  ...
  const SignOut = () => {
    ...
    if (ok) {
      ...
      // Clear any refs
      if (anchor.current) anchor.current = null;
      if (personAnchor.current) personAnchor.current = null;
  ...
  const getNotifications = async () => {
    const response = await fetch(
      `${auth.URL}/notifications/${auth?.user?.user_id}`
    );
  ...
  const viewNotifications = async () => {
    setOpen(true);
    const response = await fetch(
      `${auth.URL}/view_notifications/${auth?.user?.user_id}`
    );
        ...
        >
          ...
          <Box>
            ...
            {auth?.user?.role == "student" && (
              <IconButton
                ref={personAnchor}
                onClick={() => dispatch(toggle_is_open())}
              >
      ...
      >
        ...
        <Box sx={{ px: 3, pb: 3 }}>
          {notifications.length == 0 ? (
            <Typography className="my-3">No notifications found</Typography>
          ) : (
            notifications?.map((notification) => (
              <Alert
                key={notification.id}
        ...
        <Box sx={{ px: 2, pt: 3, pb: 2 }}>
          ...
          >
            <Typography>
              {programmes.length > 0
                ? programmes?.find(
                    (programme) => programme.id == auth?.user?.programme
                  )?.name
                : "Course not found"}
            </Typography>
          </Button>
